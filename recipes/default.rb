...
if (! data_bag.nil? && ! data_bag.strip.empty?)
then
  ...
  if (! mail_cfg.nil?)
  then
    node.set['ssmtp']['mailhub']['port'] = mail_cfg['port'] if mail_cfg.has_key?('port')

    node.set['ssmtp']['auth']['username'] = mail_cfg['username'] if mail_cfg.has_key?('username')
    node.set['ssmtp']['auth']['password'] = mail_cfg['password'] if mail_cfg.has_key?('password')
    node.set['ssmtp']['auth']['method']   = mail_cfg['auth_method'] if mail_cfg.has_key?('auth_method')

    node.set['ssmtp']['tls']['use_tls'] = mail_cfg['use_tls'] if mail_cfg.has_key?('use_tls')
    node.set['ssmtp']['tls']['use_starttls']  = mail_cfg['use_starttls']  if mail_cfg.has_key?('use_starttls')
    node.set['ssmtp']['tls']['tls_auth_cert'] = mail_cfg['tls_auth_cert'] if mail_cfg.has_key?('tls_auth_cert')
    node.set['ssmtp']['tls']['tls_auth_key']  = mail_cfg['tls_auth_key']  if mail_cfg.has_key?('tls_auth_key')
  end
end
...
template "#{node['ssmtp']['conf_dir']}/revaliases" do
  ...
  group  "root"
  mode   "0644"
  variables( :aliases => node['ssmtp']['aliases'], :mailhub => mailhub )
end
