...
platforms.each { |i| i.each_pair do |p,v|
  describe 'ssmtp2::default' do
    ...
    it 'installs the revaliases file' do
      ...
      if ! chef_run.node['ssmtp']['aliases'].empty?
      then
        chef_run.node['ssmtp']['aliases'].each_pair do |k,v|
         expect(chef_run).to render_file(file).with_content("#{k}:#{v}:#{mail_host}:#{mail_port}")
        end
      end
    end
  end
